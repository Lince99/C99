...
#define MAXTHREAD 8 // numero massimo thread
...
*/
    ...
    thread = (pthread_t*) malloc(MAXTHREAD * sizeof(pthread_t));

    for (num_thrd = 1; num_thrd <= MAXTHREAD; num_thrd++)
    {
        ...
        printf("N.Thread: %d Time: %f\n",num_thrd, dt.tv_sec + dt.tv_usec/1000000.0);
